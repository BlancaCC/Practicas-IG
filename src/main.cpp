...
void FGE_PulsarLevantarTecla( GLFWwindow* window, int key, int scancode, int action, int mods )
{
   ...
   {
      // COMPLETAR: Práctica 5: procesar la tecla 'key' para actualizar la colección de fuentes
   ...
   switch ( key )
   {
      ...
      case GLFW_KEY_J :
         cv.modo_seleccion =  ! cv.modo_seleccion ;
         cout << "modo de seleccion : " << (cv.modo_seleccion ? "activado" : "desactivado") << endl << flush ;
         break ;
